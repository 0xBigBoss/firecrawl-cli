...
describe("urlToFilePath", () => {
  ...
  it("should convert simple path to .md file", () => {
    expect(
      urlToFilePath("https://example.com/about", "https://example.com")
    ).toBe("./crawls/example.com/about.md");
  });

  it("should handle nested paths", () => {
    expect(
      urlToFilePath("https://example.com/docs/guide", "https://example.com")
    ).toBe("./crawls/example.com/docs/guide.md");
  });

  it("should remove .html extension", () => {
    expect(
      urlToFilePath("https://example.com/page.html", "https://example.com")
    ).toBe("./crawls/example.com/page.md");
  });

  it("should remove trailing slash", () => {
    expect(
      urlToFilePath("https://example.com/docs/", "https://example.com")
    ).toBe("./crawls/example.com/docs.md");
  });
  ...
  it("should preserve query params in filename (edge case)", () => {
    ...
    expect(
      urlToFilePath("https://example.com/search?q=test", "https://example.com")
    ).toBe("./crawls/example.com/search.md");
  });
});
...
describe("calculateRelativePath", () => {
  it("should calculate relative path from same directory", () => {
    expect(
      calculateRelativePath(
        ...
        "crawls/example.com/docs/api.md"
      )
    ).toBe("api.md");
  });
  ...
  it("should calculate relative path between siblings", () => {
    expect(
      calculateRelativePath(
        "crawls/example.com/docs/guide.md",
        "crawls/example.com/api/reference.md"
      )
